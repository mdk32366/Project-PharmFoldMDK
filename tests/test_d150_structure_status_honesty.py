...
def test_the_reserved_map_bars_151_and_the_pointer_moves_in_this_commit():
    ...
    # ⚠ 152 is now SPENT and its row was retired MARKER-SAFE, so this lookup keeps working and
    # the citation it protects still resolves; 153 takes the bar.
    assert re.search(r"^\| \*\*D-152\*\*", RESERVED, re.M), (
        ...
        "the surface-navigation ship spent the integer, and deleting or striking it would open a "
        "citation hole indistinguishable from D-062's")
    assert re.search(r"^\| \*\*D-153\*\*", RESERVED, re.M), (
        "D-153 is cited in order to bar it, so it must be a RESERVED row")
    assert re.search(r"^\| \*\*D-148\*\*", RESERVED, re.M), "the trafficking hold lost its row"
    assert not re.search(r"^\| ~~\*\*D-15[0123]\*\*~~", RESERVED, re.M), (
