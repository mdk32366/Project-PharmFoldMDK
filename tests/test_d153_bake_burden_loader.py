...
def test_the_next_free_integer_is_named_and_both_holds_stay_barred():
    ...
    barred; `### D-165` takes the next-free bar. ⚠ Nothing
    ...
    # `### D-165` takes the bar. Never relaxed to a `>=`.
    ...
    # NAMED above, so `### D-165` takes the next-free bar. Never relaxed to a `>=`.
    assert 148 not in ids and 165 not in ids
    ...
    # it is NAMED here rather than barred and `### D-165` takes the next-free bar. This is the
    ...
    # here rather than barred and `### D-165` takes the next-free bar. A name is ADDED and
...
def test_the_inherited_guards_were_widened_by_adding_a_name_and_never_by_relaxing():
    ...
    for rel in ("tests/test_d129_phase5_named_refuse_spec.py",
        ...
        # ⚠ D-154 spent 154, so these two files NAME it and bar 155 instead. The check moves
        ...
        assert r'\n### D-165" not in' in text, f"{rel} does not bar the next free integer"
...
def test_the_pointer_moved_in_this_commit_and_skips_both_holds():
    ...
    assert "Next free `D-` integer: **`D-165`**" in RESERVED
