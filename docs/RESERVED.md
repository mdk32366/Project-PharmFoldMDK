...
## Next-free pointers — the namespaces registered on 2026-09-04

⚠⚠ **Registered because they were NOT.** Measured 2026-09-04: `DEP-`, `P-` and `S-` each carried
written entries and **no next-free pointer anywhere in the repository**. ⚠ **An untracked namespace
cannot go stale, because nothing measures it — which is quieter than a wrong pointer and is not
better.** Four of six namespaces were in that state.

⚠ **Values are the owner's ruling of 2026-09-04** — highest spent plus one, adopted deliberately
after Code reported that gaps are not always free (`F-050` and `F-015` prove it). ⚠ **Code measured;
it did not choose.**

    Next free `DEP-` integer: `DEP-007`
    Next free `P-` integer: `P-006`
    Next free `S-` integer: `S-006`

⚠⚠ **`D-` and `F-` are NOT repeated here.** Their pointers live in the reserved-unwritten paragraph
below and are the only copy. **A second copy in this section is `F-039`'s class — a record that can
drift from the record it duplicates** — and this file exists to prevent exactly that.

⚠ **`A-` is deliberately unregistered.** The `A-0NN (descriptive name)` convention below is for
numbers that **MOVE**, so "the pointer exceeds every spent integer" is the wrong rule for it.
**Guarded by an exemption with a stated reason in `tests/test_namespace_pointers.py`, not by
silence.**

⚠ **`scripts/preflight.py` reads these. `python scripts/preflight.py` prints every namespace and
its state in seconds, from any checkout, for any team.**
