...
|---|---|---|---|
| **D-124** | ADC-C (pipeline / Right-to-Try) — the parallel Trinity lane. ⚠ **Not written here.** D-125 cites it only as the gate D-125-A/B code BUILD waits on (`D-124 A+B` on `main`). Reserving the integer so that citation is not a D-062 hole. | D-125 Spec, 2026-09-05 | **The ADC-C GO writes `### D-124`.** This reservation is not authorisation to build ADC-C, and D-125 must not spend the integer. |
