...
| **F-047** | *The wrong-but-plausible answer* — the family of results that are clean, well-formed and about the wrong thing | Owner ruling, 2026-08-19; ⚠ **renumbered from the `F-045` named in `PREWORK-2026-08-19.md` §3** | **Planner writes it.** ⚠ Lowest priority, blocks nothing. Measured members already available: `F-045`'s stale-bytecode false green · the inequality no corpus of 4,990 documents can discriminate (`F-046`) · a case-mismatched join returning a clean zero three times · a `Staged` secret answering `0` rather than erroring · **a `405` read as a `404`** · five wrong files, every one well-formed data from the right organisation. ⚠ **Its own text must state the renumbering**, or a reader following the prework's `F-045` lands on a real entry with the wrong content — which is `F-044`. |
