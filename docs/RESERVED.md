...
# ⚠ EVERY COUNT STATES ITS KEY. These three numbers were reported daily for weeks and none of
# them said what it counted — and on 2026-08-19 `cited` was predicted to rise by someone who had
# run this command the same morning. `cited` is a SET of distinct numbers, so re-citing an
# existing number cannot move it. The prediction was arithmetic about the wrong object.
print(f'defined  {len(defined):4d}  | key: `### ` headers in docs/README.md — HEADERS, not entries'
      f' (a `#### ` sub-entry is deliberately not counted)')
print(f'reserved {len(reserved):4d}  | key: `| **X-NNN**` rows in docs/RESERVED.md')
print(f'cited    {len(cited):4d}  | key: DISTINCT numbers appearing anywhere in docs/README.md +'
      f' ARCHITECTURE.md. A SET, not a total — re-citing an existing number cannot move it.')
...
| **D-146** | *Nothing yet* — the next free `D-` integer, barred by name in the nine next-free guards. | `D-145`, 2026-09-09 | **The next entry.** ⚠ Listed here only because `D-145` cites it in order to bar it; a bare `### D-146` reddens nine tests until an entry claims it by name. ⚠ **Bar OR name, never neither** — the third state (an integer neither barred nor named) is how the #266/#267 collision got in. |
