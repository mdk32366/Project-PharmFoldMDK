...
### D-128 Spec (this PR; T-1144–T-1152) — linker / seam honesty

Hermetic docs tests in `tests/test_d128_linker_seam_spec.py`. **Docs
only.** No Fly. No GPU. No ops run. No restitch of the 27. No
`hold48_*.py` edit — the D-125, D-126, and D-127 Kabsch modules are
sha256-pinned by the suite, and D-128's own module does not exist yet.
...
| **T-1152** | **PR split + hard stops:** ship index and Spec §8 distinguish **Spec (this PR)** from D-128-A / D-128-B (later Emma GOs; **CPU, no rent** in A; Kaylee does not BUILD until the Spec is on `main`); PLAN + ARCHITECTURE point at D-128; hard stops are written; **no `hold48_*.py` edit** (D-125 / D-126 / D-127 modules sha256-pinned, D-128 module absent) | `test_ship_index_distinguishes_spec_from_ab_build` · `test_plan_and_architecture_point_at_d128` · `test_hard_stops_and_not_ab` · `test_this_spec_pr_does_not_edit_hold48_modules` |
