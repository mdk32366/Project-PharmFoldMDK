# SPEC v2 — 2026-09-17 · PDB coverage census over the census, step 1, in tranches
...
> ⚠⚠ **v2 SUPERSEDES v1 IN FULL.** v1 carried **Planner error 15**: its §4 wrote *"the remaining
> 3,385"* (`3,467 − 82`), a subtraction valid only if the cohort were a **subset** of the census.
> **Measured: only 75 of the 82 are census members.** ⚠ That is `F-082`'s conflation — two
> partially-overlapping populations treated as one — **committed inside a document whose §1 makes the
> sum a hard requirement.** A pipeline built to v1's wording **would have failed its own sum check.**
> **Corrected at §1.1, §2 and §4 below. Found by Code, who re-derived the sets rather than quoting
> them. Credited.**
...
### 1.1 ⚠⚠ The populations OVERLAP. Every sum is stated PER POPULATION, never once across both.

**Measured from the committed files** (`data/cohort_82_accessions.txt`, comments skipped;
`data/census/census_manifest.v7.csv`, column `census_accession`), and **re-derived independently by both
parties rather than inherited:**

| quantity | measured |
|---|---|
| cohort accessions | **82** distinct |
| census accessions | **3,467** distinct |
| **cohort ∩ census** | **75** |
| **cohort NOT in the census** | **7** — `P51801`, `Q16880`, `Q6UXF1`, `Q6ZNA5`, `Q7Z5N4`, `Q8N4M1`, `Q9UJA9` |
| census NOT in the cohort | **3,392** |
| union — distinct proteins | **3,474** |

⚠ **The seven cohort-only accessions are NAMED because *"75 of 82"* is the half of the statement that
travels.** The other half — **7 cohort targets are not census members at all** — is the half that gets
dropped, and dropping it is how the `3,385` error happened.

**Therefore, binding on every tranche:**

1. ⚠⚠ **TWO sum checks, not one.** The seven outcomes sum to **82** for the cohort pass **and** to
   **3,467** for the census pass. ⚠ **NEVER to 3,474** — that double-counts the 75.
2. **§4 item 3's stop condition applies to each sum separately.**
3. **After T1, census accessions not yet read = 3,392**, not 3,385. ⚠ **The figure 3,385 appears nowhere
   in the instrument, and a test fails if it ever does.**
4. **The 75 and the 300 are READ TWICE** — once in T1/T2, once in T3 — and ⚠ **the artifact marks a
   re-read AS a re-read.** On a cached public-API read this is cheap, and it makes T3's sum check exact
   rather than dependent on a population that does not exist yet.
...
| **T3** | ⚠ **the FULL per-tranche census** (1×1,307 · 2×535 · 3×517 · 4×332 · 5×776 = **3,467**), **not a balance** | **3,467** | Batched on the manifest's own tranching, so a stop loses **one batch**, not the run. ⚠⚠ **v1 called this a "remainder" and also gave the full per-tranche figures — specified two ways at once.** The balance is **NOT COMPUTABLE**: the v2 sample of 300 is drawn in v2 Stage 0 and **does not exist**, so any balance today would be a guess. ⚠ **Tranche 5 is the rental/tiled population** — report it separately; its spans are the least PDB-tractable. |
...
⚠⚠ **T2's 300 is a NAMED DEPENDENCY, not a number.** It is drawn in v2 Stage 0. **If it is not drawn when
this work reaches T2, T2 is SKIPPED and T3 runs, and the skip is recorded.** ⚠ T3 does not wait on it,
because T3's population is the full census (§2, T3 row) and does not depend on T2 at all.
...
4. **More than 5 of the 82 land in `accession_unresolved` or `span_unknown`** — that points at our record,
   not at the PDB, and it is investigated before the census pass is pulled. ⚠⚠ **v1 said "the remaining
   3,385" here. That figure is VOID (see the v2 banner and §1.1). The census pass reads 3,467; the
   accessions not yet read after T1 number 3,392.**

5. ⚠ **The cohort sum check fails** — the seven outcomes do not sum to **82** — or **the two sum checks
   are reported as one.** §1.1 makes them separate, and collapsing them is the error v2 exists to
   correct.
...
3. ⚠⚠ **The categories sum to the population, and the sum is asserted — SEPARATELY for the cohort pass
   (82) and the census pass (3,467).** A test **fails** if the two are combined into one check, and a
   test **fails** if the literal `3385` or `3,385` appears anywhere in the instrument (§1.1 item 3).
