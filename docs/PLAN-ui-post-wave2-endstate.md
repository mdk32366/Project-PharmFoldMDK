...
> seam ≈ **88.76 Å**. Kabsch park → **D-125** (Spec + A + B on `main`). Overlap-confidence Kabsch Spec is **D-126** (docs + A + B on `main`). Piecewise / domain-aware Kabsch Spec is **D-127** (already on `main`, `00fa76d` / #243). **D-127-A** already shipped the core BUILD (`e49bf34` / #244; `piecewise_kabsch/`; no trim; CPU, no rent). **D-127-B** already shipped UI four-path honesty + the mandatory Spec §7 Method addendum (`de9a80e` / #245). Linker / seam honesty Spec is **D-128** (already on `main`, `2004c5a` / #246). **D-128-A** already shipped the core BUILD (`9e65cbf` / #247; `linker_seam/`; §1a honesty rows for every path; ±32 aa window; no pieces, no trim; CPU, no rent). **D-128-B** is UI five-path honesty + the mandatory Spec §7 Method addendum (this PR), which also discloses the D-128 OPS result as recorded: PASS 0 / REFUSE 7 / FAIL 0, `repaired_of_seven` = 0, give-back 5 vs D-125 and 6 vs D-126. Phase 5 named-refuse Spec is **D-129** (this PR; [`SPEC-phase5-named-refuse.md`](SPEC-phase5-named-refuse.md)) — **labels, not algorithms**: **eight** parents (**2938, 2939, 3179, 3190, 3321, 3368, 3566** + **3432**) are **`accept-refuse`**, never **open must-hunt** / **solved** / **a D-128 miss**; that OPS disclosure is **standing** and may not be softened or split apart; **3272 / 3394** stay **Phase 4 must-hunt** on a **separate Matt GO**; **no linker-v2**; served stays **assembler**; **10.0 Å stays**. ⚠ The Method D-128-B shipped still called the seven **must-hunt** — that re-label was owed to a later Method / UI PR, not the D-129 docs Spec PR, and **D-129-B** (this PR) ships it: the eight are labelled **named refuse / `accept-refuse`** on the owner Method, `/method`, and the review card, from one signed fate registry whose accept-refuse block **carries** the D-128 OPS 0 of 7 and the 5 / 6 give-back. **3272 / 3394** render an **open** Phase 4 fate and are never labelled accepted. No surface may
...
| Piecewise / domain-aware Kabsch | Spec is **D-127** (already on `main`). **D-127-A** already shipped the core BUILD (`e49bf34` / #244; `piecewise_kabsch/`; no trim; CPU, no rent). **D-127-B** already shipped UI four-path honesty + the mandatory Spec §7 Method addendum (`de9a80e` / #245; per-piece rows, never a seam average; default served = assembler) and disclosed the D-127 OPS result (PASS 17 / REFUSE 10 / FAIL 0; 0 of 3 recovered; regress 5 vs D-125, 7 vs D-126). 10.0 Å gate stays | [`SPEC-piecewise-domain-kabsch.md`](SPEC-piecewise-domain-kabsch.md) |
| Linker / seam honesty | Spec is **D-128** (already on `main`). §1a requires a per-path / per-seam `max_ca_jump_angstrom` — over **10.0 Å** = **dishonest** for that seam, fail-closed, no success PDB presented as honest. §1b's optional repair is **linker-local rigid only** (**±32 aa** window, **W = 32**, ε = 1e-3, **no trim loop**, no pieces, no linker-inherit) → existing `winning_tile`; sibling `linker_seam/{parent_id}/`. Primary inventory = the **seven** signed must-hunt linker parents (2938, 2939, 3179, 3190, 3321, 3368, 3566 — ⚠ **`must-hunt` is what they were called when D-128 was written; the Phase 5 sign has since superseded that name and they are `accept-refuse`, D-129 / D-129-B**). **Not piecewise-v2; not an RMSD Spec (3272 / 3394 out of primary); not a domain Spec; 3432 stays accept-refuse.** Never says solved — goal is **diagnose / refuse dishonest seams**. **D-128-A** already shipped the core BUILD (`9e65cbf` / #247; honesty rows for all four trees, each naming its source; post-apply `seam_jump_gt_10` measured across the **whole** seam). **D-128-B** (this PR) reads that tree for five-path UI honesty — one row per `(path, seam)`, never an average — and ships the mandatory Method addendum, including the D-128 OPS result **as recorded** (**PASS 0 / REFUSE 7 / FAIL 0**; `repaired_of_seven` = **0**, the pre-registered allowed outcome; `seam_jump_gt_10` ×6 / `rmsd_gt_10` ×1; named give-back 5 vs D-125 and 6 vs D-126; ⚠ not re-measured there). 10.0 Å gate stays and does not loosen without Matt; served stays assembler; **D-126 remains best experimental until proven otherwise**; the **D-127 failed experiment stays disclosed** | [`SPEC-linker-seam-honesty.md`](SPEC-linker-seam-honesty.md) ⚠ **§3 / §9 now carry the D-129 Phase 5 cross-link: the seven are `accept-refuse`, not must-hunt** — a **label** change only; §1a / §1b / §2 / §5 / §11 stand as shipped. **D-129** (this PR) is that labelling authority: the **eight** (seven + 3432) are **named refuse**, the shipped OPS disclosure becomes **standing**, **3272 / 3394** stay **Phase 4 must-hunt** on a separate Matt GO, and there is **no linker-v2** |
| Phase 4 residual RMSD | Spec is **D-130** (already on `main`, `854c2ab` / #252; [`SPEC-residual-rmsd-hunt.md`](SPEC-residual-rmsd-hunt.md)), under the **Emma/Matt GO Phase 4 RMSD 2026-09-05 ~20:39 PT** (*"Go phase 4"*) that D-129 §6 required. **Algorithm authority**, docs only there. **D-130-A** (**this PR**) is the core BUILD, under the **Emma BUILD GO Phase 4 residual RMSD 2026-09-06** (Matt via Emma; the **#252 merge is the GO**): `core/hold48_residual_rmsd.py` + `scripts/residual_rmsd_restitch.py`, sixth sibling `residual_rmsd/{parent}/`, CPU / stdlib / no rent, **no `hold48_*.py` edit**, **no UI**, **no Method edit**, **no ops run**. It records three readings the Spec left open — a prior path's recorded `rmsd_angstrom` is **never** copied into `rigid_rmsd_angstrom`, the floor is measured **once per seam from the pre-transform input tiles**, and **`irreducible` is evaluated before `unknown`** — in `### D-130-A`. ⚠ **D-130-B (UI + the mandatory Spec §7 Method addendum) has NOT shipped**, so D-130 is not "done", and the **OPS** run of the two is a third GO. ⚠ **One failure mode — residual RMSD** (`rmsd_gt_10`, whole overlap) — and **two parents only: 3272** `Q6V0I7` **/ 3394** `Q8TDW7`, both **Phase 4 must-hunt**, both absent from the D-128 OPS seven. **Not a linker Spec, not a domain-partition Spec, not both, not a kitchen sink.** §1a's required decomposition records the **proved floor** `dRMSD / 2` beside each path's achieved full-overlap RMSD and classes it `irreducible` / `placement` / `unknown`; ⚠ **the floor is one-directional** — above 10.0 Å it certifies impossibility, below it, it proves nothing and never argues for a looser gate. §1b's optional recovery is a **residue-identity** correspondence audit plus **D-125's fit unchanged** — **no trim** (the D-126 lie surface, which hid a 28–68 Å gap on 3272), **no weights, no pieces, no window, no linker-inherit**. **`recovered_of_two` = 0 is pre-registered as allowed**; a **named refuse after a failed hunt** is a complete outcome. Sixth sibling `residual_rmsd/{parent}/` + `core/hold48_residual_rmsd.py`, colliding with no earlier name. ⚠ **Phase 5 is not reopened** — the eight stay `accept-refuse` and D-129 §4's disclosure stays standing; **10.0 Å stays; served stays assembler; no auto-flip; no F-004; D-126 remains best experimental and callable; never solved without measurement**. **D-130-A** / **D-130-B** / the **OPS** run each need their own GO | [`SPEC-residual-rmsd-hunt.md`](SPEC-residual-rmsd-hunt.md) ⚠ **D-129 §6 now carries the D-130 cross-link:** the pair's hunt is **Spec-governed**, and they are still **not** `accept-refuse` |
...
Kabsch park → **D-125** (Spec on `main`; confirm `### D-125` exists). D-125-A is the core BUILD (on `main`, `26a40a8` / #237). **D-125-B** is the UI dual-path honesty BUILD (confirm `### D-125-B` exists). Overlap-confidence follow-on Spec is **D-126** (confirm `### D-126` exists; [`SPEC-overlap-confidence-kabsch.md`](SPEC-overlap-confidence-kabsch.md)). **D-126-A** is the core BUILD (confirm `### D-126-A` exists). **D-126-B** is the UI triple-path honesty BUILD (confirm `### D-126-B` exists). Piecewise / domain-aware follow-on Spec is **D-127** (confirm `### D-127` exists; [`SPEC-piecewise-domain-kabsch.md`](SPEC-piecewise-domain-kabsch.md)). **D-127-A** is the core BUILD (confirm `### D-127-A` exists). **D-127-B** is the UI four-path honesty + mandatory Method BUILD (confirm `### D-127-B` exists). Linker / seam honesty follow-on Spec is **D-128** (confirm `### D-128` exists; [`SPEC-linker-seam-honesty.md`](SPEC-linker-seam-honesty.md)) — §1a per-path seam honesty is required, §1b's linker-local rigid repair is optional and **not** piecewise-v2. **D-128-A** is the core BUILD (confirm `### D-128-A` exists). **D-128-B** is the UI five-path honesty + mandatory Method BUILD (confirm `### D-128-B` exists). Phase 5 named-refuse Spec is **D-129** (confirm `### D-129` exists; [`SPEC-phase5-named-refuse.md`](SPEC-phase5-named-refuse.md)) — **labelling authority only**: the **eight** parents are **`accept-refuse`**, **3272 / 3394** stay **Phase 4 must-hunt** on a **separate Matt GO**, the D-128 OPS **0 of 7** disclosure is **standing**, and there is **no linker-v2**. **D-129-B** is the Method / UI labelling BUILD that discharges its §3 re-label (confirm `### D-129-B` exists), and **D-129-C** is the hygiene patch on the retired name (confirm `### D-129-C` exists). Phase 4 residual-RMSD hunt Spec is **D-130** (confirm `### D-130` exists; [`SPEC-residual-rmsd-hunt.md`](SPEC-residual-rmsd-hunt.md)) — **algorithm authority**, one failure mode (`rmsd_gt_10`, whole overlap), two parents (**3272 / 3394**, still **Phase 4 must-hunt** and still **not** `accept-refuse`), a **one-directional** proved floor as the required half, **D-125's fit unchanged** as the only permitted refit, and **`recovered_of_two` = 0 pre-registered as allowed**. **D-130-A** is the core BUILD (confirm `### D-130-A` exists) — sixth sibling `core/hold48_residual_rmsd.py` + `scripts/residual_rmsd_restitch.py`, no `hold48_*.py` edit, no UI, no Method edit, no ops run. **D-130-B** (UI + the mandatory Method addendum) and the **OPS** run of the two are two separate later GOs, and **B is not authorised yet**. This plan is still not itself a BUILD GO.
