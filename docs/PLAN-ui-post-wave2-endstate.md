...
| Linker / seam honesty | Spec is **D-128** (**this PR**, docs only). §1a requires a per-path / per-seam `max_ca_jump_angstrom` — over **10.0 Å** = **dishonest** for that seam, fail-closed, no success PDB presented as honest. §1b's optional repair is **linker-local rigid only** (**±32 aa** window, **W = 32**, ε = 1e-3, **no trim loop**, no pieces, no linker-inherit) → existing `winning_tile`; sibling `linker_seam/{parent_id}/`. Primary inventory = the **seven** signed must-hunt linker parents (2938, 2939, 3179, 3190, 3321, 3368, 3566). **Not piecewise-v2; not an RMSD Spec (3272 / 3394 out of primary); not a domain Spec; 3432 stays accept-refuse.** Never says solved — goal is **diagnose / refuse dishonest seams**. **D-128-A** (core) and **D-128-B** (UI + mandatory Method) are later Emma GOs. 10.0 Å gate stays and does not loosen without Matt; served stays assembler; **D-126 remains best experimental until proven otherwise**; the **D-127 failed experiment stays disclosed** | [`SPEC-linker-seam-honesty.md`](SPEC-linker-seam-honesty.md) |
