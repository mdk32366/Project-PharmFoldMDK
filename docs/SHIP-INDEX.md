...
> [`decisions.md`](decisions.md). Write new decisions there first (append-at-top). This
...
- **D-130-A ships the Phase 4 residual-RMSD core BUILD** (**this PR**, off
  ...
  **T-1200**–**T-1209**). Draft PR; **no self-merge**; **Trinity merges**.
  Full entry: `### D-130-A` in [`decisions.md`](decisions.md).
- **D-130 already shipped the Phase 4 residual-RMSD hunt Spec** on `main`
  ...
  **B's has not**, and the **OPS** run of the two is a third. Full entry:
  `### D-130` in [`decisions.md`](decisions.md).
- **D-129-C already shipped the `must-hunt` supersession hygiene patch
  ...
  **Twelve** bare occurrences at `cbcb47d` were qualified by **adding** a
  ...
  of [`decisions.md`](decisions.md), 3 here, and 1 in
...
- **D-129-B already shipped the Phase 5 named-refuse LABELS on Method +
  ...
  **untouched** and still guarded by content.
  ...
  closes. Full entry: `### D-129-B` in [`decisions.md`](decisions.md).
- **D-129 already shipped the Phase 5 named-refuse Spec** on `main`
  ...
  **hunt**, not the **record**.
  ...
  Full entry: `### D-129` in [`decisions.md`](decisions.md).
...
- **D-128-A already shipped the linker / seam honesty core BUILD** on
  ...
  **Kaylee does not BUILD** until the Spec is on `main` **and** an
  ...
  at `### D-128-A` in [`decisions.md`](decisions.md).
...
Full entries: [`decisions.md` § D-130](decisions.md#d-130--phase-4-residual-rmsd-hunt-certify-the-floor-before-claiming-a-fit--3272--3394-only-recover-with-honesty-or-refuse-by-name-docs-only),
[`decisions.md` § D-129](decisions.md#d-129--phase-5-named-refuse-the-eight-linker--seam-parents-are-accept-refuse-the-d-128-rescues-0-of-7-stays-disclosed-and-the-stitch-family-freezes-docs-only),
[`decisions.md` § D-128-B](decisions.md#d-128-b--ui-linker--seam-path-honesty--the-mandatory-d-128-method-addendum),
[`decisions.md` § D-128-A](decisions.md#d-128-a--linker--seam-honesty-core-1a-per-path-seam-honesty-rows--optional-32-aa-linker-local-rigid-then-existing-winning_tile),
[`decisions.md` § D-128](decisions.md#d-128--linker--seam-honesty-spec-diagnose-and-refuse-dishonest-seams-optional-linker-local-rigid-docs-only),
[`decisions.md` § D-127-B](decisions.md#d-127-b--ui-four-path-honesty--the-mandatory-d-127-method-addendum),
[`decisions.md` § D-127-A](decisions.md#d-127-a--piecewise--domain-aware-kabsch-core-per-domain-weighted-fit-no-trim-then-existing-winning_tile),
[`decisions.md` § D-127](decisions.md#d-127--piecewise--domain-aware-kabsch-spec-multi-rigid-fit-then-existing-winning_tile-docs-only),
[`decisions.md` § D-126-B](decisions.md#d-126-b--ui-triple-path-honesty-name-assembler-d-125-kabsch-path-and-d-126-confidence-kabsch-artifacts-without-colliding-them),
[`decisions.md` § D-126-A](decisions.md#d-126-a--overlap-confidence-kabsch-core-trimmed--plddt-weighted-fit-then-existing-winning_tile),
[`decisions.md` § D-126](decisions.md#d-126--overlap-confidence-kabsch-spec-trimmed--plddt-weighted-fit-then-existing-winning_tile-docs-only),
[`decisions.md` § D-125-B](decisions.md#d-125-b--ui-dual-path-honesty-name-assembler-and-kabsch-path-artifacts-without-colliding-them),
[`decisions.md` § D-125](decisions.md#d-125--kabsch-restitch-spec-overlap-cα-align-then-existing-winning_tile-stitch-d-125-a-core-build).
...
| Id | Role | Ships? |
| --- | --- | --- |
| **D-143** | **Track B structural-only ranking copy** — the owner Doc's Track B sentence (`docs/pharmfold-adc-nectin4-paper.md`) and its D-123 verbatim extract (`ui/src/aboutPaper.js` `TRACK_B`) stop describing an aspirational cancer / normal / internalization / density composite as **what we rank by**, and describe what is computed: **structure only — membrane × ECD × fold confidence (pLDDT)**. The four biology terms are named as **excluded** (⚠ **not** defaulted to 0.5 neutrals — the owner rejected that as dishonest), the order refuses **ADC readiness** in its own words, and its scope is stated: the whole census (**3,467** rows, not the tranche-5 slice), running **offline**. ⚠⚠ **NOT the cohort-82 D-041/D-060 learned scorer** — `core/scorer.py`, `scripts/fit_scorer.py`, `target_scores`, the `/targets` **Rank** column, `/scorer` and MethodNote's prose about that scorer are a different surface over a different population and are untouched. No route, component, payload field, migration, ops, fold, F-004 or ranking-set change | **Yes — this PR.** Docs + UI copy + tests only, under the **owner GO 2026-09-08 (Matt)**. Amends **D-123** in place (its own quotation of the retired composite is left standing with a supersession note — D-129-C). ⚠ **Shipped first as `D-142` and renumbered by owner instruction 2026-09-09**; **142 is held**, registered in [`RESERVED.md`](RESERVED.md) — nothing visible in the tree spends it, so the six id guards keep its bar and add 143 beside it. ⚠ Also carries a **test-only race fix** the gate caught in `MethodNote.toc.test.jsx` (helper awaited the static rail, then asserted on async coverage copy); the assertion was not relaxed. |
