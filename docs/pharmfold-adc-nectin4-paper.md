...
*⚠ Amended 2026-09-08 (owner GO; logged as **D-143** in [`README.md`](README.md)). Track B's ranking sentence above used to end with a product of cancer expression, membrane topology, internalization and antigen density over normal-tissue risk. That was the **aspiration**, not the computation: the four biology terms have no data behind them yet, and filling them with 0.5 neutrals so the product would evaluate was rejected as dishonest — a placeholder that is multiplied in comes out looking like a measurement. The sentence above is what the ranking actually is. The old wording is not restored, and the note is kept rather than the claim.*

*⚠ Amended 2026-09-09 (owner GO; logged as **D-146** in [`README.md`](README.md)). Track B's ranking sentence above used to end "and it runs offline: it is not a ranked surface in this application." That was true when **D-143** was written and is **false now**: **D-144** put the structural score in the database and on its own route, **D-145** baked the loader into the serving image, and the order is served by this application. Measured from this build rather than asserted — `GET https://pharmfoldmdk.fly.dev/api/census-structural-ranking` on 2026-09-09 returns `result_status: valid` over **3,467** rows, run 1 computed at `2026-09-09T06:02:08Z`, with rank 1 GABBR2 (`O75899`) at **0.8443**. **What moved is where the number lives, not what the number is:** it is still membrane × ECD × fold confidence, still STRUCTURAL_ONLY, still not ADC readiness, still not a shortlist, and still not the cohort-82 learned scorer. The D-143 note above stands unchanged; neither the offline wording nor the retired composite is restored.*
