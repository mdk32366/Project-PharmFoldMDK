...
| `data/adcs/` | ADC-A v1 FDA-approved catalog (`adcs.v1.json`, D-119). ADC-B pages (`/adcs` Approved shelf, D-122) consume it via `GET /api/adcs`. ADC-C-A (D-124) adds sibling `adcs.pipeline.v1.json` + `access.v1.json` (not merged into v1). ADC-C-B (D-124) consumes those siblings on `/adcs` Pipeline + Access. Weekly Drugs@FDA watch is Emma's ops lane — see that folder's README. |
