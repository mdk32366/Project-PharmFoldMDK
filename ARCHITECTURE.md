...
> D-121 does not add `/adcs` and must not regress those routes.
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   [`docs/decisions.md`](docs/decisions.md) indexes which id **ships** (D-121 ships
   the Method hold-48 8th-grade explainer; D-123 ships the Nectin Doc →
   AdcContext follow-on, already on `main`; D-122 ships ADC-B `/adcs`; D-120
   ships Phase 2 review; D-119 ships ADC-A; D-118 ships P0 honesty; D-117 is
   the parent PLAN). Do not claim Phase 2 as D-119. #229 stays merged. D-121
   does not add `/adcs`.
