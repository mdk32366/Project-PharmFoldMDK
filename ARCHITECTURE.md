...
| **ADC catalog v1** (D-119 / ADC-A) | Dated **FDA-approved drug** roster, separate from the scorer mapping. `data/adcs/adcs.v1.json`: currently marketed ADCs that resolved on Drugs@FDA **2026-09-05** (15 rows; floor, not a census). Every field is `{value, source, as_of, confidence}` (`official` / `reviewed` / `derived`). openFDA is authority for approval identity; antigen / UniProt are reviewed (D-029 seam). No DAR / efficacy / chemistry numbers. No pipeline / RTT (ADC-C). No `/adcs` UI (ADC-B). Emma's weekly Drugs@FDA watch is a documented hook (`data/adcs/README.md`), not a gate. Served by `GET /api/adcs` and `GET /api/adcs/{adc_id}`. | `core/adc_catalog.py` — pure, fixture-tested; `data/adcs/adcs.v1.json` |
