...
> thin read API; no UI). **D-125 ships** the Kabsch restitch **Spec** (docs only;
> [`docs/SPEC-kabsch-restitch.md`](docs/SPEC-kabsch-restitch.md); already on
> `main`, `fbe8978` / #234) — overlap Cα Kabsch → existing `winning_tile`
> stitch; seams not solved. **D-125-A/B** code BUILD is later and waits until
...
> edit `adcs.v1.json` and does not add `/adcs` React routes. D-125 does not
> change the stitch path.
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   ships ADC-C-A pipeline + access/RTT data + API; D-125 ships the Kabsch
   restitch Spec, docs only, already on `main` — D-125-A/B code BUILD is
   later and waits on D-124 A+B; D-121 ships the Method hold-48 8th-grade
   ...
   claim Phase 2 as D-119. Do not treat D-125 Spec as a stitch BUILD.
