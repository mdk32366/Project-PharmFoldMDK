...
> tree; does not edit `hold48_kabsch.py`. **D-126-B** is the UI
> triple-path honesty BUILD (this PR): reads A's tree; names assembler /
> D-125 `kabsch/` / D-126 `confidence_kabsch/` when present; default
> served PDB stays assembler. D-125 Kabsch + assembler
> remain the production dual-path and stay callable. D-124 A+B is on
...
> artifacts exist (honest absence when missing). Production dual-path
> is unchanged.
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   callable); D-126-B ships UI triple-path honesty; D-121 ships the Method hold-48 8th-grade
