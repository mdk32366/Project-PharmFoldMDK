...
| **ADC catalog v1** (D-119 / ADC-A, D-122 / ADC-B) | Dated **FDA-approved drug** roster, separate from the scorer mapping. `data/adcs/adcs.v1.json`: currently marketed ADCs that resolved on Drugs@FDA **2026-09-05** (15 rows; floor, not a census). Every field is `{value, source, as_of, confidence}` (`official` / `reviewed` / `derived`). openFDA is authority for approval identity; antigen / UniProt are reviewed (D-029 seam). No DAR / efficacy / chemistry numbers. Pipeline / RTT live in **sibling files** (D-124), never in this one. **D-122 / ADC-B** is the `/adcs` + `/adcs/:id` React consumer (sortable name / cancer-type-absence / protein index + baseball card; cancer type is the named v1 absence — indication is not an ADC-A field). **D-124 / ADC-C-B** keeps that Approved shelf and adds a Pipeline shelf beside it — not a merge. Emma's weekly Drugs@FDA watch is a documented hook (`data/adcs/README.md`), not a gate. Served by `GET /api/adcs` and `GET /api/adcs/{adc_id}`. | `core/adc_catalog.py` — pure, fixture-tested; `data/adcs/adcs.v1.json`; `ui/src/components/AdcsView.jsx` + `AdcCard.jsx` |
