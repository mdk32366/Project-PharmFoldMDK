...
> `repaired_of_seven` is claimed for no parent, and **0-of-7 is an
> allowed outcome**. **D-128-B** (UI path honesty + the **mandatory**
> Spec §7 Method addendum) is a later Emma GO; **A does not discharge
> it**, and no D-128 surface may say seams are solved. D-124 A+B is on
...
> exist (honest absence when missing) and discharges the Spec §7
> Method obligation. The **served** path is still the assembler, so
