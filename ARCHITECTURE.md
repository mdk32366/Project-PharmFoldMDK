...
> that re-label is the one piece of B-side work D-129 leaves owed — and
> it is **not** in this docs Spec PR. ⚠ **`accept-refuse` ≠ Method
...
> retires the **hunt**, not the **record**. D-129 is **docs only**: no
> UI, no `MethodNote.jsx`, no Method file edit (its §5 8th-grade excerpt
> is **authority** a later Method PR discharges), no `hold48_*.py` edit,
> no ops run. D-124 A+B is on
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   sibling path (`9e65cbf` / #247), with D-128-B (UI + mandatory Method)
   still owed; D-129 ships the Phase 5 named-refuse **labelling** Spec
   (this PR — **eight** parents `accept-refuse`, **3272 / 3394** stay
   **Phase 4 must-hunt** on a separate Matt GO, **no linker-v2**);
