...
> [`docs/SPEC-kabsch-restitch.md`](docs/SPEC-kabsch-restitch.md)) — overlap Cα
> Kabsch → existing `winning_tile` stitch; seams not solved. **D-125-A/B**
> code BUILD is later and waits until **D-124 A+B** (ADC-C) is on `main`.
> **D-121 ships** the Method hold-48 8th-grade explainer (`docs/method-hold48-tiles.md`
> + additive `/method` section). **D-123 ships** the Nectin-4/ADC Doc follow-on on
> `/about` (already on `main`, `2ffd4f8` / #231). **D-122 ships** ADC-B (`/adcs`
> UI) on `main` (`86f8a10` / #232). **D-120 ships** Phase 2 review UI; parent
> honesty GO is **D-118**; parent PLAN is **D-117**. **D-119 ships** ADC-A.
> D-125 does not change the stitch path.
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   [`docs/decisions.md`](docs/decisions.md) indexes which id **ships** (D-125
   ships the Kabsch restitch Spec, docs only — D-125-A/B code BUILD is later
   and waits on D-124 A+B; D-121 ships the Method hold-48 8th-grade explainer;
   D-123 ships the Nectin Doc → AdcContext follow-on, already on `main`;
   D-122 ships ADC-B `/adcs`; D-120 ships Phase 2 review; D-119 ships ADC-A;
   D-118 ships P0 honesty; D-117 is the parent PLAN). Do not claim Phase 2
   as D-119. Do not treat D-125 Spec as a stitch BUILD.
