...
> solved. **D-125-B** (this PR) is **UI dual-path honesty only**. D-124 A+B is on
...
> when artifacts exist. The served assembler path is unchanged.
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   the core BUILD that feeds `winning_tile` — D-125-B (this PR) is UI
   dual-path honesty only; D-121 ships the Method hold-48 8th-grade
