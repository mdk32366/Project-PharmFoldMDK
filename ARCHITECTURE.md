...
> `main`, `fbe8978` / #234) **and D-125-A** (this PR): overlap Cα Kabsch →
> transform tile → existing `winning_tile` via `core/hold48_kabsch.py`;
> seams not solved. **D-125-B** (UI dual-path) is later. D-124 A+B is on
...
> flagged pre-stitch path; the served assembler path is unchanged.
...
2. **Every design decision is written into [`docs/README.md`](docs/README.md) *before*
   ...
   already on `main` and D-125-A (this PR) as the core BUILD that feeds
   `winning_tile` — D-125-B UI is later; D-121 ships the Method hold-48 8th-grade
