...
| **Scorer** (D-015 §3, D-041, D-060) | The learned ADC-suitability model, **by hand in pure stdlib** (`numpy`/`scipy`/`sklearn` are in no lock file). `core/scorer.py`: L2-regularized logistic regression over the six standardized features — **seven parameters** (six coefficients + intercept), fit by **IRLS** (Newton on the penalized log-likelihood; non-convergence **raises**, never a silent estimate). Evaluation is pre-registered and every free parameter fixed before a result exists (D-060): **no RNG** (deterministic stratified folds), a **13-point λ grid** selected by 5-fold inner CV *inside each LOO fold*, leave-one-out reporting a **distribution** of held-out-positive percentiles over the ranking set (56, F-002), a **Spearman** vs the evidence-score comparator, and a head-to-head on one **common reference set** (the comparator is two-valued → degenerate by construction). **The LOO runs first and independently of the ranking-table full-data fit, and a fold that fails to converge is recorded (named, `converged=False`) without aborting the loop** — the distribution is reported over the folds that converged, with the non-convergent count and names carried (D-063). Under quasi-complete separation (n=12 / 6 features) the unpenalized-intercept Hessian is singular at the CV-selected low-edge λ, so a raise is correct behaviour, not a bug; the intercept stays unpenalized and the grid is never extended (D-063 refusals). ⚠ **Label (Group B) and comparator (evidence score) are different quantities and never mix** — a scrambled-comparator fixture must give byte-identical coefficients. Attribution is `β_k·x_k` per target (D-041 dec 1). `scripts/fit_scorer.py`: assembles rows from `protein_features` + labels + evidence, runs the evaluation, and on `--run --persist` writes `target_scores` + `ranking_results` (D-061) and stamps the `ranking_run`. **Written and fixture-tested; the fit itself is a separate owner-authorised run** — the labels are curated (D-040 pass, 2026-07-27) but the first fit produces a recorded result the moment it runs. | `core/scorer.py` (pure, ships, fixture-tested incl. leakage guards); `scripts/fit_scorer.py` (offline, excluded from the image) |
...
- **`ranking_results`** (D-061, `db.models.RankingResult`, migration **`0004`**) — one row per run:
  ...
  `plddt_floor`), the per-fold λ + grid-edge flag, the `excluded` set with reasons, and
  `scorer_version` + `feature_version`. Both tables are additive (migration `0004`, verified by
