...
describe('MethodNote — D-121 hold-48 8th-grade explainer (additive, not a gut)', () => {
  ...
  it('labels the eight named refuse / accept-refuse and keeps the numbers beside the label', async () => {
    ...
    // Phase 4 pair now labelled on the same page as the accepted eight.
    expect(t).toMatch(/3272/)
    expect(t).toMatch(/3394/)
    expect(t).toMatch(/Phase 4 pair/)
    expect(t).toMatch(/named refuse \/ accept-refuse/)
    expect(t).toMatch(/no longer/)
    expect(t).not.toMatch(/still being looked at/)
    expect(t).not.toMatch(/Phase 4 must-hunt/)
  ...
  // D-130-B / D-131 — Phase 4 residual-RMSD OPS disclosure + named refuse.
  it('discloses Phase 4 OPS 0/2 and labels 3272/3394 accept-refuse', async () => {
    getCoverage.mockResolvedValue(FIXTURE)
    const { container, getByTestId } = renderMethod()
    await waitFor(() => expect(container.textContent).toMatch(/3 ranked-and-folded of 7/))
    const t = getByTestId('phase4-named-refuse-addendum').textContent
    expect(t).toMatch(/932292d/)
    expect(t).toMatch(/residual_rmsd_ops_2026-09-05/)
    expect(t).toMatch(/PASS 0/)
    expect(t).toMatch(/REFUSE 2/)
    expect(t).toMatch(/recovered_of_two/)
    expect(t).toMatch(/rmsd_irreducible/)
    expect(t).toMatch(/12\.63/)
    expect(t).toMatch(/rmsd_gt_10/)
    expect(t).toMatch(/13\.77/)
    expect(t).toMatch(/4\.77/)
    expect(t).toMatch(/3272/)
    expect(t).toMatch(/3394/)
    expect(t).toMatch(/named refuse \/ accept-refuse/)
    expect(t).toMatch(/17 PASS/)
    expect(t).toMatch(/10 accept-refuse/)
    expect(t).toMatch(/assembler/)
    expect(t).toMatch(/10\.0/)
    expect(t).toMatch(/D-126/)
    expect(t).toMatch(/no longer Phase 4 must-hunt/)
    expect(t).not.toMatch(/still being looked at/)
    expect(t).not.toMatch(/They are Phase 4 must-hunt/)
    expect(t).toMatch(/Never claim 27\/27 PASS/)
    expect(t).toMatch(/Never claim the seams are solved/)
    expect(t).not.toMatch(/the seams are fixed|full-length AF-quality/)
  })
